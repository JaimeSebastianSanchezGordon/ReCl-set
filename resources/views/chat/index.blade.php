...
@section('content')
    <div class="mx-auto max-w-4xl">
        ...
        @if ($conversations->isEmpty())
            {{-- ===== Estado vacío ===== --}}
            <div class="rounded-3xl border border-stone-200/80 bg-white p-16 text-center shadow-xl shadow-stone-100/50">
                ...
                <div class="mt-8">
                    <a
                        ...
                        class="inline-flex items-center rounded-xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white shadow-md shadow-emerald-600/10 transition hover:bg-emerald-700 hover:shadow-emerald-750/20 active:scale-95"
                    >
                        Explorar armario
                    </a>
                </div>
        ...
        @else
            ...
            <div class="overflow-hidden rounded-3xl border border-stone-200/80 bg-white shadow-xl shadow-stone-100/40">
                <div class="divide-y divide-stone-100">
                    @foreach ($conversations as $conversation)
                        ...
                        @endphp
                        <div
                            class="group block p-5 hover:bg-stone-50/70 transition duration-200 relative {{ $isUnread ? 'bg-emerald-50/10' : '' }}"
                        >
                            <div class="flex items-center gap-4">
                                ...
                                >
                                    ...
                                    <div class="h-16 w-16 shrink-0 overflow-hidden rounded-2xl border border-stone-200 bg-stone-50 flex items-center justify-center shadow-sm">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-baseline justify-between gap-2">
                                            <h4 class="text-sm font-bold text-stone-900 group-hover:text-emerald-700 transition">
                                                {{ $otherUser->name }}
                                            </h4>
                                        ...
                                        <div class="flex items-center gap-1.5 mt-0.5 text-xs font-semibold text-emerald-800">
                                            <span>{{ $garment ? $garment->name : 'Prenda eliminada' }}</span>
                                            @if($garment)
                                                <span class="h-1 w-1 rounded-full bg-stone-300"></span>
                                                <span class="bg-emerald-50 px-1.5 py-0.5 rounded text-emerald-700 font-bold">${{ number_format((float) $garment->price, 2) }}</span>
                                            @endif
                                        </div>
                                        
                                        <p class="mt-1.5 text-sm text-stone-500 truncate leading-relaxed {{ $isUnread ? 'font-semibold text-stone-800' : '' }}">
                                            @if ($lastMessage)
                                                ...
                                                {{ $lastMessage->body }}
                                            @else
                                                <span class="italic text-stone-400/90 font-normal">Conversación iniciada. Envía un mensaje.</span>
                                            @endif
                                        </p>
                                    </div>
                                </a>

                                {{-- Botón de eliminación y estado (aparece al pasar el mouse en desktop) --}}
                                <div class="flex items-center gap-3 shrink-0">
                                    {{-- Punto verde de no leído con animación de pulso --}}
                                    @if ($isUnread)
                                    ...
                                    {{-- Botón de basura con diseño premium --}}
                                    <button
                                        type="button"
                                        ...
                                        class="md:opacity-0 group-hover:opacity-100 inline-flex h-9 w-9 items-center justify-center rounded-xl text-stone-400 hover:bg-rose-50 hover:text-rose-600 transition duration-200"
                                    ...
                                    {{-- Flecha indicadora --}}
                                    <a href="{{ route('chat.show', $conversation) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-xl hover:bg-stone-100 text-stone-450 transition">
    ...
    {{-- ===== Modal de confirmación de eliminación (compartido y animado) ===== --}}
    <div
